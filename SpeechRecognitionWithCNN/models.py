...
# ==============================================================================
"""Model architecture definitions for simple speech recognition """
...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  ####################################
  # Get number of samples for different audio stores
  ####################################

  # The number of samples per: audio clip
  desired_samples = int(sample_rate * clip_duration_ms / 1000)

  # The number of samples per: window
  window_size_samples = int(sample_rate * window_size_ms / 1000)

  # The number of samples per: window stride
  window_stride_samples = int(sample_rate * window_stride_ms / 1000)
  
  # Difference in size between audio clip and window
  length_minus_window = (desired_samples - window_size_samples)
  
  # If the analysis window is larger than the full clip
  if length_minus_window < 0:
    spectrogram_length = 0 # The spectrogram will have length 0
  else: # else, the spectrogram length will be
  ...
  # Something about a finger print?
  
  # Return dictionary containing common settings
  return {
...
#######################################################################
# General helper function to create different kinds of models
'''by calling their respective constructor functions'''
#######################################################################
...
#######################################################################
# Create a single hidden fully-connected layer
#######################################################################
...
#######################################################################
'''Default model architecture for this script'''
def create_conv_model(fingerprint_input, model_settings, is_training):
    ...
    # Set convolution filters
    first_filter_width = 8
    first_filter_height = 20
    first_filter_count = 64
    first_weights = tf.get_variable(
        name='first_weights',
        initializer=tf.truncated_normal_initializer(stddev=0.01), # Normal distribution
        shape=[first_filter_height, first_filter_width, 1, first_filter_count])
    
    # Set bias
    first_bias = tf.get_variable(
        name='first_bias',
        initializer=tf.zeros_initializer, # Set to zero
        shape=[first_filter_count])
    
    # Convolution of finger-prints and weights
    first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, 1, 1, 1], 'SAME') + first_bias
    
    # Non-linear activation function (relu)
    first_relu = tf.nn.relu(first_conv)
    
    # Perform dropout with a given probability (if training)
    if is_training:
    ...
    # Convert filter matrices into a single, flattened vector
    ...
    # Create weights from flattened weight vector to output neurons
    label_count = model_settings['label_count']
    ...
    # Create biases for output neurons
    final_fc_bias = tf.get_variable(
        name='final_fc_bias',
        initializer=tf.zeros_initializer,
        shape=[label_count])

    # Multiply final, filter kernel data by final weights and add a bias
...
#######################################################################
# Create a convolutional model with low compute requirements.
#######################################################################
...
#######################################################################
# Create an SVDF model with low compute requirements.
#######################################################################
...
#######################################################################
# Create a convolutional model aimed at microcontrollers.
#######################################################################
